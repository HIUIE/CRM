...
| # | 任务 | 目标 | 优先级 |
|---|------|------|--------|
| 1 | **多语言支持 (i18n)** | 适配外贸场景，支持中/英文切换，统一 UI 文案管理 | 高 |
| 2 | **全文搜索优化** | 利用 PostgreSQL `tsvector` 实现订单、客户、任务的全局极速模糊搜索 | 高 |
| 3 | **Docker 化交付** | 提供标准 `Dockerfile` 和 `compose`，屏蔽环境差异，实现一键部署 | 中 |
| 4 | **审计日志 UI** | 增加管理员可视化的操作审计界面，支持按时间、操作员、对象筛选 | 中 |
| 5 | **实时协作通知** | 引入 WebSocket (Socket.io)，实现任务指派、订单状态变更的即时推送 | 中 |
| 6 | **AI OCR 增强** | 识别订单附件（PDF/图片），自动提取关键财务数据并回填利润表 | 低 |
| 7 | **监控与指标** | 暴露 `/metrics` 接口（Prometheus 格式），监控内存/连接池/接口延迟 | 低 |
| 8 | **可视化看板 2.0** | 优化 Dashboard 交互，增加年度利润对比、地区分布等动态图表 | 低 |
