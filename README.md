...
### 方式一：Docker 部署（推荐，无需手动装数据库，Windows/macOS/Linux 通用）

**前提：** 安装 [Docker Desktop](https://www.docker.com/products/docker-desktop/)（Windows 版安装后会自动启用 WSL2）
...
#
# 用记事本/VS Code 打开 .env，修改：
#   - JWT_SECRET（改成随机字符串，如 k8s9d7f3h2j5m1n4p6q0r2t8）
#   - INITIAL_ADMIN_PASSWORD（管理员登录密码）

# 3. 一键启动（自动下载 PostgreSQL 和运行环境）
docker compose up -d
```

> 首次启动需要下载镜像，耗时 1-3 分钟，之后秒开。

默认访问：`http://localhost:3000`
管理员账号：`root` / 你设置的 `INITIAL_ADMIN_PASSWORD`

---

### 方式二：本地部署（需自行安装 PostgreSQL）
...
# 安装完成后打开 "SQL Shell (psql)"，执行：CREATE DATABASE smarttrade_crm;
...
#   - PG_PASSWORD（你在安装 PostgreSQL 时设置的密码）
...
# 4. 配置环境变量
cp .env.example .env
# 编辑 .env，至少修改 JWT_SECRET、INITIAL_ADMIN_PASSWORD
# 如果 PostgreSQL 设置了密码，修改 PG_PASSWORD
...
- **后端**：Express 5 + **PostgreSQL 16** (原生支持异步路由)
...
备份范围：PostgreSQL 数据库 + uploads 附件目录
默认路径：`data/backups/`
