...
*   **敏感导出审计 (New)**：附件下载、业务归档包、系统灾备包与 Excel 导出均记录审计日志，方便管理员追溯资料流向。
...
*   **稳定性自愈**：前端资源加载失败、旧版本 chunk、运行时渲染错会自动恢复一次；API 查询加入短重试，后台健康检查覆盖数据库、附件目录和构建产物。
...
*   **安全审计修复**：AI 订单分析、订单批量更新、合作伙伴及联系人接口已补充服务端归属校验；附件下载、导出与备份已纳入审计。
