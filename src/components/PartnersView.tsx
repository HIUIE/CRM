  ...
  return (
    <div className="space-y-4">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {showForm ? (
          <form onSubmit={handleSubmit} className="mt-5 rounded-3xl border border-slate-100 bg-slate-50 p-6">
            <div className="mb-6 text-[11px] font-bold text-primary-navy uppercase tracking-widest">{editingPartner ? `编辑伙伴档案：${editingPartner.name}` : '新增伙伴档案'}</div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              ...
              <Field label="伙伴类型 *">
                <select value={form.partnerType} onChange={(event) => setForm({ ...form, partnerType: event.target.value as PartnerType })} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-primary-navy transition-colors outline-none appearance-none cursor-pointer">
              ...
              <Field label="联系人">
                <input value={form.contactPerson} onChange={(event) => setForm({ ...form, contactPerson: event.target.value })} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-primary-navy transition-colors outline-none" />
              </Field>
              <Field label="联系电话/邮箱">
                <input value={form.contact} onChange={(event) => setForm({ ...form, contact: event.target.value })} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-primary-navy transition-colors outline-none" />
              </Field>
              <Field label="国家 / 地区">
                <input value={form.country} onChange={(event) => setForm({ ...form, country: event.target.value })} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-primary-navy transition-colors outline-none" />
              </Field>
              <div className="lg:col-span-2">
                <Field label="详细地址">
                  <input value={form.address} onChange={(event) => setForm({ ...form, address: event.target.value })} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-primary-navy transition-colors outline-none" />
                </Field>
              </div>
              <Field label="付款条款">
                <input value={form.paymentTerms} onChange={(event) => setForm({ ...form, paymentTerms: event.target.value })} placeholder="例如：月结 30 天..." className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-primary-navy transition-colors outline-none" />
              </Field>
              <div className="md:col-span-2 lg:col-span-3">
                <Field label="备注">
                  <textarea value={form.remark} onChange={(event) => setForm({ ...form, remark: event.target.value })} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-primary-navy transition-colors outline-none" rows={2} />
                </Field>
              </div>
            </div>
            <div className="mt-8 flex justify-end gap-3">
              ...
              <button type="submit" className="rounded-xl bg-primary-navy px-10 py-2.5 text-sm font-bold text-white hover:bg-slate-800 transition-all shadow-md">保存伙伴</button>
            </div>
          </form>
        ) : null}
      </section>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
        {loading ? (
          <div className="text-sm text-slate-500 p-8">正在读取伙伴数据...</div>
        ) : (
                    ...
                    filteredPartners.map((partner) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-4">
                          <div className="font-bold text-primary-navy uppercase tracking-tight">{partner.name}</div>
                          <div className="text-[11px] text-slate-400 mt-1 uppercase font-medium">{partner.country || 'GLOBAL'}</div>
                        </td>
                        <td className="px-4 py-4">
                        ...
                        <td className="px-4 py-4">
                           <div className="text-slate-700 font-bold">{partner.contact_person || '未填写'}</div>
                           <div className="text-[11px] text-slate-500 truncate max-w-[200px]">{partner.contact}</div>
                        </td>
                        <td className="px-4 py-4" onClick={(e) => e.stopPropagation()}>
                          <div className="flex justify-end gap-2">
                            <button onClick={() => openEdit(partner)} className="rounded-lg border border-slate-200 p-2 text-secondary-slate transition-all hover:bg-white hover:text-primary-navy hover:border-slate-300">
                              <Edit className="h-4 w-4" />
                            </button>
                            {user?.role === 'admin' ? (
                              <button onClick={() => void handleDelete(partner)} className="rounded-lg border border-slate-200 p-2 text-slate-300 transition-all hover:bg-red-50 hover:text-red-600 hover:border-red-200">
