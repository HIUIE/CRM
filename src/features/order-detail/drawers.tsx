    ...
    <div className="space-y-12">
      ...
      <section className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <Field label="订单结算币种 *">
            <select value={orderForm.currency} onChange={e => setOrderForm({ ...orderForm, currency: e.target.value as OrderFormState['currency'] })} className="w-full bg-slate-50 dark:bg-navy-950 p-3 text-[14px] font-bold text-primary-navy dark:text-white appearance-none focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm">
          ...
          {orderForm.items.map((item, idx) => (
            <div key={item.clientKey} className="relative p-5 bg-slate-50 dark:bg-navy-950/50 rounded-lg border border-slate-200 dark:border-navy-800 group hover:bg-surface dark:hover:bg-navy-800 hover:border-primary-navy/30 dark:hover:border-tertiary-sage/30 transition-all shadow-sm">
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid gap-10 sm:grid-cols-2">
        <Field label="生产记录日期"><input type="date" value={productionLogForm.logDate} onChange={e => setProductionLogForm({ ...productionLogForm, logDate: e.target.value })} className="w-full bg-surface dark:bg-navy-950 py-2.5 px-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm" /></Field>
      </div>
...
export function CustomsForm({
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid gap-8 sm:grid-cols-2">
        <Field label="报关当前状态">
          <select value={customsForm.status} onChange={e=>setCustomsForm({...customsForm, status:e.target.value as any})} className="w-full bg-surface dark:bg-navy-950 p-3.5 text-[14px] font-bold text-primary-navy dark:text-white appearance-none focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm cursor-pointer">
        ...
        <Field label="合作报关行"><input value={customsForm.brokerName} onChange={e=>setCustomsForm({...customsForm, brokerName:e.target.value})} placeholder="输入报关行全称..." className="w-full bg-surface dark:bg-navy-950 p-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm data-field" /></Field>
        <Field label="正式报关单号"><input value={customsForm.declarationNo} onChange={e=>setCustomsForm({...customsForm, declarationNo:e.target.value})} placeholder="输入 18 位海关报关单号..." className="w-full bg-surface dark:bg-navy-950 p-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm data-field" /></Field>
        <Field label="贸易方式分类"><select value={customsForm.tradeMode} onChange={e=>setCustomsForm({...customsForm, tradeMode:e.target.value})} className="w-full bg-surface dark:bg-navy-950 p-3.5 text-[14px] font-bold text-primary-navy dark:text-white appearance-none focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm cursor-pointer"><option value="一般贸易">一般贸易 (0110)</option><option value="进料加工">进料加工 (0615)</option><option value="来料加工">来料加工 (0214)</option><option value="其他">其他类型</option></select></Field>
        <Field label="海关申报日期"><input type="date" value={customsForm.declarationDate} onChange={e=>setCustomsForm({...customsForm, declarationDate:e.target.value})} className="w-full bg-surface dark:bg-navy-950 py-2.5 px-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm" /></Field>
        <Field label="海关放行日期"><input type="date" value={customsForm.releaseDate} onChange={e=>setCustomsForm({...customsForm, releaseDate:e.target.value})} className="w-full bg-surface dark:bg-navy-950 py-2.5 px-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm" /></Field>
      </div>
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid gap-10 sm:grid-cols-2">
        ...
        <Field label="实际承运商 (Actual Carrier)"><input required value={logisticsForm.carrier} onChange={e=>setLogisticsForm({...logisticsForm, carrier:e.target.value})} placeholder="例如: 顺丰 / 马士基 / DHL..." className="w-full bg-surface dark:bg-navy-950 py-2.5 px-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm" /></Field>
        <Field label="主运单/提单识别码 *"><input required value={logisticsForm.trackingNo} onChange={e=>setLogisticsForm({...logisticsForm, trackingNo:e.target.value})} placeholder="请输入单号..." className="w-full bg-surface dark:bg-navy-950 p-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm data-field" /></Field>
        <Field label={logisticsForm.segmentType === 'domestic' ? '实际发货日期' : '预计离港日期'}><input type="date" value={logisticsForm.shippingDate} onChange={e=>setLogisticsForm({...logisticsForm, shippingDate:e.target.value})} className="w-full bg-surface dark:bg-navy-950 py-2.5 px-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm" /></Field>
        <Field label="当前物流节点状态"><select value={logisticsForm.status} onChange={e=>setLogisticsForm({...logisticsForm, status:e.target.value as LogisticsStatus})} className="w-full bg-surface dark:bg-navy-950 p-3.5 text-[14px] font-bold text-primary-navy dark:text-white appearance-none focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm cursor-pointer"><option value="preparing">01. 待起运 (Pre-shipment)</option><option value="shipped">02. 运输中 (In-transit)</option><option value="arrived">03. 已妥投 (Delivered)</option></select></Field>
        <Field label="贸易术语 (Incoterms)"><input value={logisticsForm.incoterm} onChange={e=>setLogisticsForm({...logisticsForm, incoterm:e.target.value})} placeholder="FOB Shenzhen / CIF Hamburg / EXW..." className="w-full bg-surface dark:bg-navy-950 py-2.5 px-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm data-field" /></Field>
        <Field label="运输方式"><input value={logisticsForm.transportMode} onChange={e=>setLogisticsForm({...logisticsForm, transportMode:e.target.value})} placeholder="海运 / 空运 / 快递 / 铁运..." className="w-full bg-surface dark:bg-navy-950 py-2.5 px-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm" /></Field>
        {logisticsForm.segmentType === 'international' && (
          <>
            <Field label="船名/航次 (Vessel/Voyage)"><input value={logisticsForm.vesselVoyage} onChange={e=>setLogisticsForm({...logisticsForm, vesselVoyage:e.target.value})} placeholder="MAERSK HANOI / 426W" className="w-full bg-surface dark:bg-navy-950 py-2.5 px-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm data-field" /></Field>
            <Field label="提单号 (B/L No.)"><input value={logisticsForm.billNo} onChange={e=>setLogisticsForm({...logisticsForm, billNo:e.target.value})} placeholder="BL-..." className="w-full bg-surface dark:bg-navy-950 py-2.5 px-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm data-field" /></Field>
            <Field label="ETD 预计开船"><input type="date" value={logisticsForm.etd} onChange={e=>setLogisticsForm({...logisticsForm, etd:e.target.value})} className="w-full bg-surface dark:bg-navy-950 py-2.5 px-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm" /></Field>
            <Field label="ETA 预计到港"><input type="date" value={logisticsForm.eta} onChange={e=>setLogisticsForm({...logisticsForm, eta:e.target.value})} className="w-full bg-surface dark:bg-navy-950 py-2.5 px-3.5 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm" /></Field>
          </>
        )}
        {logisticsForm.segmentType === 'domestic' && <div className="sm:col-span-2"><Field label="最终收货/卸货地址 *"><textarea rows={3} value={logisticsForm.recipientAddress} onChange={e=>setLogisticsForm({...logisticsForm, recipientAddress:e.target.value})} className="w-full bg-surface dark:bg-navy-950 p-4 text-[14px] font-bold text-primary-navy dark:text-white focus:outline-none rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm leading-relaxed" /></Field></div>}
      </div>
            ...
            <div className="grid gap-6 sm:grid-cols-4 items-end">
              <div className="sm:col-span-3">
                <div className="grid gap-4 grid-cols-4 font-bold">
                  <Field label="件数 (箱)"><input type="number" value={item.packageCount} onChange={e => { const next = [...packingForm.items]; next[idx].packageCount = e.target.value; setPackingForm({ items: next }); }} className="w-full bg-surface dark:bg-navy-950 p-2.5 rounded-lg border border-slate-200 dark:border-navy-800 outline-none data-field text-primary-navy dark:text-white" /></Field>
                  <Field label="尺寸 (L*W*H)"><input value={item.packageSize} onChange={e => { const next = [...packingForm.items]; next[idx].packageSize = e.target.value; setPackingForm({ items: next }); }} placeholder="120x100x80" className="w-full bg-surface dark:bg-navy-950 p-2.5 rounded-lg border border-slate-200 dark:border-navy-800 outline-none data-field text-primary-navy dark:text-white" /></Field>
                  <Field label="毛重 (kg)"><input value={item.grossWeight} onChange={e => { const next = [...packingForm.items]; next[idx].grossWeight = e.target.value; setPackingForm({ items: next }); }} className="w-full bg-surface dark:bg-navy-950 p-2.5 rounded-lg border border-slate-200 dark:border-navy-800 outline-none data-field text-primary-navy dark:text-white" /></Field>
                  <Field label="净重 (kg)"><input value={item.netWeight} onChange={e => { const next = [...packingForm.items]; next[idx].netWeight = e.target.value; setPackingForm({ items: next }); }} className="w-full bg-surface dark:bg-navy-950 p-2.5 rounded-lg border border-slate-200 dark:border-navy-800 outline-none data-field text-primary-navy dark:text-white" /></Field>
                </div>
              </div>
              <div className="flex flex-col items-center gap-2">
                <label className="h-14 w-14 rounded-lg border-2 border-dashed border-slate-300 dark:border-navy-700 bg-surface dark:bg-navy-950 flex items-center justify-center cursor-pointer hover:border-primary-navy dark:hover:border-tertiary-sage hover:bg-slate-50 transition-all overflow-hidden shadow-sm">
...
export function AIAnalysisContent({
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-1000">
      <div className="relative flex items-center justify-between overflow-hidden rounded-lg border border-slate-200 bg-surface p-8 shadow-sm dark:border-navy-800 dark:bg-navy-900">
        <div>
      ...
      <section className="space-y-5 px-2">
        ...
        <div className="space-y-4">
          {aiResult.risks.map((r, i) => <div key={i} className="rounded-lg border border-red-100 bg-surface p-6 text-[16px] font-bold leading-relaxed text-slate-700 shadow-sm transition-all hover:border-red-200 dark:border-red-900/30 dark:bg-navy-900 dark:text-slate-300">“{r.content}”</div>)}
        </div>
      </section>
    </div>
  );
