    ...
    <div className="mx-auto max-w-6xl space-y-8 pb-12 animate-page-in">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {sections.map((section, idx) => (
          <div key={idx} className="rounded-lg border border-slate-200 dark:border-navy-800 bg-surface dark:bg-navy-900 p-8 shadow-sm transition-all hover:shadow-md">
            ...
            <h2 className="mb-6 text-xl font-bold text-primary-navy dark:text-white tracking-tight">{section.title}</h2>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-lg border border-slate-200 dark:border-navy-800 bg-surface dark:bg-navy-900 p-8 shadow-sm transition-colors">
          ...
          <div className="space-y-4">
             <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-navy-950/50 rounded-lg border border-slate-100 dark:border-navy-800">
                <div>
                   <div className="text-xs font-bold text-slate-400 tracking-tight mb-1">开发者邮箱</div>
             ...
             <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-navy-950/50 rounded-lg border border-slate-100 dark:border-navy-800">
                <div>
                   <div className="text-xs font-bold text-slate-400 tracking-tight mb-1">内部协作群</div>
        ...
        <section className="rounded-lg border border-slate-200 dark:border-navy-800 bg-surface dark:bg-navy-900 p-8 shadow-sm transition-colors">
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50/70 p-5 dark:border-navy-800 dark:bg-navy-950/50">
             <div className="mb-3 flex items-center gap-3 text-emerald-700 dark:text-emerald-400">
                <ShieldCheck size={18} />
                <span className="text-xs font-bold tracking-tight">已启用 TLS 传输加密</span>
             </div>
             <p className="text-xs font-medium leading-relaxed text-slate-500 dark:text-slate-400">所有与 AI 引擎的交互均通过加密通道传输，系统不存储您的 API 访问详情。</p>
          </div>
        </section>
      </div>
    </div>
