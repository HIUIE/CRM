    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {AI_PROVIDERS.map(p => (
            <button key={p.id} onClick={() => handleProviderChange(p.id)} className={`flex items-center gap-3 p-4 rounded-lg border transition-all ${selectedProvider === p.id ? 'border-primary-navy bg-primary-navy/5' : 'border-slate-200 bg-white hover:border-slate-300'}`}>
              <div className={`p-2 rounded-lg ${selectedProvider === p.id ? 'bg-primary-navy text-white' : 'bg-slate-100 text-slate-500'}`}>{p.icon}</div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="模型选择">
          <select value={model} onChange={(e) => setModel(e.target.value)} className="w-full rounded-lg border border-slate-200 p-3.5 text-sm outline-none text-primary-navy bg-white">
        ...
        <Field label="自定义模型 ID">
          <input value={model} onChange={(e) => setModel(e.target.value)} placeholder="如 deepseek-chat" className="w-full rounded-lg border border-slate-200 p-3.5 text-sm outline-none text-primary-navy bg-white" />
        </Field>
      </div>

      <Field label="API 访问密钥 (API Key)" className="max-w-lg">
        <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="留空表示沿用现有密钥" className="w-full rounded-lg border border-slate-200 p-3.5 text-sm outline-none text-primary-navy bg-white" />
      </Field>

      <Field label="自定义 API 代理地址 (Base URL)" className="max-w-lg">
        <input type="text" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder={AI_PROVIDERS.find(p => p.id === selectedProvider)?.baseUrl || 'https://api.deepseek.com'} className="w-full rounded-lg border border-slate-200 p-3.5 text-sm outline-none text-primary-navy bg-white" />
      </Field>
      ...
      <div className="flex gap-3">
        ...
        <button onClick={testAiConnection} disabled={testingAi} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-5 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-50">
