  ...
  return (
    <div className="space-y-8 max-w-4xl">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="模型选择">
          <select value={model} onChange={(e) => setModel(e.target.value)} className="w-full rounded-lg border border-slate-200 bg-surface p-3.5 text-sm text-primary-navy outline-none transition-colors focus:border-primary-navy dark:border-navy-800 dark:bg-navy-950 dark:text-white dark:focus:border-tertiary-sage">
        ...
        <Field label="自定义模型 ID">
          <input value={model} onChange={(e) => setModel(e.target.value)} placeholder="如 deepseek-chat" className="w-full rounded-lg border border-slate-200 bg-surface p-3.5 text-sm text-primary-navy outline-none transition-colors focus:border-primary-navy dark:border-navy-800 dark:bg-navy-950 dark:text-white dark:focus:border-tertiary-sage" />
        </Field>
      </div>

      <Field label="API 访问密钥 (API Key)" className="max-w-lg">
        <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="留空表示沿用现有密钥" className="w-full rounded-lg border border-slate-200 bg-surface p-3.5 text-sm text-primary-navy outline-none transition-colors focus:border-primary-navy dark:border-navy-800 dark:bg-navy-950 dark:text-white dark:focus:border-tertiary-sage" />
      </Field>

      <Field label="自定义 API 代理地址 (Base URL)" className="max-w-lg">
        <input type="text" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder={AI_PROVIDERS.find(p => p.id === selectedProvider)?.baseUrl || 'https://api.deepseek.com'} className="w-full rounded-lg border border-slate-200 bg-surface p-3.5 text-sm text-primary-navy outline-none transition-colors focus:border-primary-navy dark:border-navy-800 dark:bg-navy-950 dark:text-white dark:focus:border-tertiary-sage" />
      </Field>
      ...
      <div className="flex gap-3">
        ...
        <button onClick={testAiConnection} disabled={testingAi} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-surface px-5 py-2.5 text-sm font-bold text-slate-700 transition-colors hover:bg-slate-50 disabled:opacity-50 dark:border-navy-700 dark:bg-navy-900 dark:text-slate-300 dark:hover:bg-navy-800">
